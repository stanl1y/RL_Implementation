...
class Maze_v2(gym.Env):
    ...
    def __init__(self, maze_size=100, random_reset=True, combine_s_g=False):
        ...
        if(self.combine_s_g):
            self.observation_space = spaces.Box(low=-maze_size, high=maze_size, shape=(2,), dtype=np.int32)
        else:
            self.observation_space = spaces.Box(low=0, high=maze_size, shape=(4,), dtype=np.int32)
    ...
    def check_legal(self,state):
        x, y = state
        if 49<=x and x<=50 and not(74<=y and y<=75 and self.current_goal==3):#check horizontal wall
            return False
        if 49<=y and y<=50 and not(24<=x and x<=25 and self.current_goal==1) and not(74<=x and x<=75 and self.current_goal==5):#check vertical wall
            return False
    ...
    def step(self, action):
        ...
        if self.combine_s_g:
            return (self.state-self.target_list[self.current_goal]), reward, self.is_terminal(), {"x":self.state[0],"y":self.state[1],"goal_idx":self.current_goal,"goal":self.target_list[self.current_goal]}
        else:
            return np.concatenate((self.state,self.target_list[self.current_goal])), reward, self.is_terminal(), {"x":self.state[0],"y":self.state[1],"goal_idx":self.current_goal,"goal":self.target_list[self.current_goal]}
    ...
    def reset(self):
        if self.random_reset:
            ...
            self.target_list=[np.array([20,20]),#key1
