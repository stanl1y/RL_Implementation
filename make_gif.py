...
parser.add_argument("-n", "--name", type=str, default="", help="which exp to make gif")
args = parser.parse_args()
images = []
images_path = []
exp_name = f"{args.env}_{args.name}"
...
for idx, filename in enumerate(data_root.joinpath("neighbor_reward").iterdir()):
    if not str(filename)[-3:] == "png":
        continue
    img = imageio.imread(filename)
    n_reward.append(img)
...
for idx, data in enumerate(zip(color_images, path_images, n_reward, arr_images)):
    color_img, path_img, n_reward_img, arr_img = data
    # reward=reward_df.loc[idx][f"{args.exp_name} - Reward"]
    reward = testing_reward[idx]
    extend_img = np.zeros(
        (
            arr_img.shape[0] + 130,
            arr_img.shape[1] + color_img.shape[1] + 10,
            arr_img.shape[2],
        )
    ).astype(np.uint8)
    extend_img[: color_img.shape[0], : color_img.shape[1]] = color_img
    extend_img[
        color_img.shape[0] + 50 : color_img.shape[0] + 50 + path_img.shape[0],
        : path_img.shape[1],
    ] = path_img
    extend_img[
        color_img.shape[0]
        + 50
        + path_img.shape[0]
        + 50 : color_img.shape[0]
        + 50
        + path_img.shape[0]
        + 50
        + n_reward_img.shape[0],
        : path_img.shape[1],
    ] = n_reward_img
    extend_img[
        : arr_img.shape[0],
        color_img.shape[1] + 10 : color_img.shape[1] + 10 + arr_img.shape[1],
    ] = arr_img
    # extend_img[-19:,:]=np.array([255,255,255,255])
    extend_img[-128:-122, : int(reward / max_reward * extend_img.shape[1])] = np.array(
        [255, 0, 0, 255]
    )
